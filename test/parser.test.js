...
test('парсит карточку из реального лога без явного "X Zimmer" рядом с числом', () => {
  // Похожий на реальный кейс из прода: число комнат может стоять отдельно
  const text = 'Zimmer: 3 | 68,01 m², 432,41 € | Havemannstraße 23, 10319 Lichtenberg';
  const apt = parseAptText(text);
  assert.ok(apt, 'должен распарситься');
  assert.strictEqual(apt.rooms, '3', 'должен извлечь число комнат из формата "Zimmer: N"');
});
