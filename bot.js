    ...
    try {
        ...
        const apartments = await page.evaluate(() => {
            ...
            document.querySelectorAll('a').forEach(a => {
                ...
                if (href && (href.includes('/expose/') || href.includes('howoge') || href.includes('gewobag') || href.includes('degewo'))) {
        ...
        if (isFirst) {
            ...
            console.log(`Отправлено ${newCount} новых`);
        }
...
async function main() {
    ...
    await sendTelegram('🤖 Бот перезапущен. Мониторим 3-комнатные до 600€.');
    await checkApartments();
    setInterval(checkApartments, CHECK_INTERVAL);
}

main().catch(e => console.error('Critical:', e));
